...
exports.start = async (options) => {
    ...
    switch (command) {
        ...
        case 'secrets': {
            ...
            const strFromPath = await Helpers.readFileAndDecryptIfNeeded(path);
            ...
            switch (subCommand) {
                ...
                case 'list':
                    ...
                    log(await Helpers.withErrHandling(Dogtag.list, strFromPath, user, search));
                    break;

                case 'add':

                    await Helpers.withErrHandling(Dogtag.add, strFromPath, user);
                    log(await Helpers.withErrHandling(Dogtag.list, strFromPath, user, search));
                    break;
                ...
                case 'remove':
                    ...
                    log(await Helpers.withErrHandling(Dogtag.delete, strFromPath, user, search));
                    break;

                case 'update':
                case 'edit':

                    log(await Helpers.withErrHandling(Dogtag.update, strFromPath, user, search));
                    break;
        ...
        case 'remote': {
            ...
            switch (subCommand) {
                ...
                case 'sync': {
                    ...
                    const syncJwt = await Helpers.withErrHandling(RemoteManager.syncRemote, remoteToSync, user, password);
                    ...
                    if (secrets.includes(DEFAULT_DOG_TAG_NAME)) {
                        ...
                        const localTag = await Helpers.withErrHandling(Dogtag.getInstance, USER_LOCAL_TAG_FILE_PATH, user);
                        ...
                        if (!diff.length) {
                            ...
                            await Helpers.withErrHandling(Dogtag.save, USER_LOCAL_TAG_FILE_PATH, user.fingerprint, mergedTag);

                            log('');
                            log('Merged remote tag with local default tag!');

                            const encryptedMergedTag = await Helpers.withErrHandling(Doggo.api.encrypt, user.fingerprint, Helpers.withErrHandling(Dogtag.getAutomergeSave, mergedTag));
