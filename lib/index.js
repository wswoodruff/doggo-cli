...
const LOCAL_TAG_FILE_PATH = (fingerprint) => `${Os.homedir()}/doggo/local-dog-tag-${fingerprint}.gpg`;
        ...
        case 'keys': {
            ...
            switch (subCommand) {
                ...
                case 'export':
                    ...
                    log(await Doggo.api.exportKey(user.fingerprint, type, output, password));
                    break;

                case 'delete':

                    user = await Helpers.pickUser(user);
                    type = type || await Helpers.prompt('Enter key type ([-k|-t] [pub|sec|all])');

                    log(await Doggo.api.deleteKeys(user.fingerprint, type));
                    break;
        ...
        case 'secrets': {
            ...
            switch (subCommand) {
                ...
                case 'list':
                    ...
                    log(await Dogtag.list(path, user.fingerprint, search));
                    break;

                case 'add':

                    await Dogtag.add(path, user.fingerprint);
                    log(await Dogtag.list(path, user.fingerprint, search));
                    break;
                ...
                case 'remove':
                    ...
                    log(await Dogtag.delete(path, user.fingerprint, search));
                    break;

                case 'update':
                case 'edit':

                    log(await Dogtag.update(path, user.fingerprint, search));
                    break;
                ...
                case 'share':
                    ...
                    // log(await RemoteManager.shareSecret(user, toShareUser.fingerprint, search));
                    break;
            }
        ...
        case 'remote':
            ...
            switch (subCommand) {
                ...
                case 'list':
                    ...
                    log('');
                    log(await RemoteManager.listRemotes(user.fingerprint));
                    break;
                ...
                case 'add':
                    ...
                    const addRemotes = await RemoteManager.listRemotes(user.fingerprint);

                    if (addRemotes.includes(remoteUrl)) {
                        throw new Error(`Remote "${remoteUrl}" already exists!`);
                    }

                    const [addSyncErr, addSyncJwt] = await RemoteManager.syncRemote(remoteUrl, user.fingerprint, password);

                    if (addSyncErr) {
                        return log('Error', addSyncErr);
                    }
                ...
                case 'sync': {
                    ...
                    const [err, remotes] = await RemoteManager.listRemotes(user.fingerprint);
                    let remoteToSync;
                    ...
                    const [syncErr, syncJwt] = await RemoteManager.syncRemote(remoteToSync, user.fingerprint, password);

                    if (syncErr) {
                        return log('Error', syncErr);
                    }
                    ...
                    if (secrets.includes(DEFAULT_DOG_TAG_NAME)) {
                        ...
                        const { output: decryptedRemoteTag, err: decryptedRemoteTagErr } = await Doggo.api.decrypt(remoteSecret);

                        if (decryptedRemoteTagErr) {
                            throw decryptedRemoteTagErr;
                        }

                        // 'decryptedRemoteTag' should now be the output of Automerge.save,
                        // so it's JSON stringified and ready to be put into Automerge.load() (used in Dogtag.getInstance)
                        const { output: remoteTag } = await Dogtag.getInstance(decryptedRemoteTag, user.fingerprint);
                        const { output: localTag } = await Dogtag.getInstance(USER_LOCAL_TAG_FILE_PATH, user.fingerprint);
                        ...
                        if (!diff.length) {
                            ...
                            await Dogtag.save(USER_LOCAL_TAG_FILE_PATH, user.fingerprint, mergedTag);

                            log('');
                            log('Merged remote tag with local default tag!');

                            const { output: encryptedMergedTag, err } = await Doggo.api.encrypt(user.fingerprint, Dogtag.getAutomergeSave(mergedTag));
                        ...
                        const addSecretRes = await RemoteManager.addSecret(syncJwt, remoteToSync, {
                            secret: await Fs.readFile(USER_LOCAL_TAG_FILE_PATH, { encoding: 'utf8' }),
                            name: DEFAULT_DOG_TAG_NAME,
                            type: 'dog-tag'
                        });

                        if (addSecretRes.error) {
                            return log(addSecretRes);
                        }
