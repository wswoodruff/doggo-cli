        ...
        case 'secrets': {
            ...
            switch (subCommand) {
                ...
                case 'add':
                    ...
                    // TODO test out if adding secrets goes to the correct path
                    // Figure out why there's no diff between local and the remote tags
        ...
        case 'remote':
            ...
            switch (subCommand) {
                ...
                case 'list':
                    ...
                    log('');
                    log(await RemoteManager.listRemotes(fingerprint));
                    break;
                ...
                case 'add':
                    ...
                    const addRemotes = await RemoteManager.listRemotes(fingerprint);

                    if (addRemotes.includes(remoteUrl)) {
                        throw new Error(`Remote "${remoteUrl}" already exists!`);
                    }

                    const [addSyncErr, addSyncJwt] = await RemoteManager.syncRemote(remoteUrl, user, password);
                ...
                case 'sync':
                    ...
                    const remotes = await RemoteManager.listRemotes(fingerprint);
                    ...
                    const [syncErr, syncJwt] = await RemoteManager.syncRemote(remoteToSync, user, password);
                    ...
                    if (secrets.includes(DEFAULT_DOG_TAG_NAME)) {
                        ...
                        const { output: remoteTag } = await Dogtag.getInstance(decryptedRemoteTag, user);
                        const { output: localTag } = await Dogtag.getInstance(USER_LOCAL_TAG_FILE_PATH, user);
                        ...
                        if (!diff.length) {
                            ...
                            await Dogtag.save(USER_LOCAL_TAG_FILE_PATH, user, mergedTag);
