...
exports.start = async (options) => {
    ...
    const log = (str) => options.out.write(`${str}\n`);
    const ctx = { options, output: log, DisplayError };
        ...
        case 'bone': {
            ...
            switch (subCommand) {
                ...
                case 'remove':
                    ...
                    console.log(await Dogtag.delete(user));
                    break;

                case 'update':

                    ({ output } = await Dogtag.update(user));

                    if (output.includes('No result found')) {
...
internals.commandArgs = {
    keys: internals.commandDescription({
        subCommands: ['gen', 'list', 'delete', 'import', 'export'],
        flags: ['type', 'output']
    }),
...
internals.doggoApiCallWithFlags = async (doggo, args, funcName, flagsConfig) => {
    ...
    Helpers.assert([doggo, args, funcName, flagsConfig].every((a) => a), new Error('(doggo, args, funcName, flagsConfig) are required'))

    const flagsConfigArr = [].concat(flagsConfig);
    const flags = flagsConfigArr.map((f) => args[f]);
